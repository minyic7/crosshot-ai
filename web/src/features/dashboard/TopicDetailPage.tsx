...
import { ArrowLeft, RefreshCw, Clock, TrendingUp, TrendingDown, Minus, Trash2, Pause, Play, Send, Languages, Loader2, Heart, Eye, Repeat2, MessageSquare, BarChart3, Image, Pencil, X, Check, CheckCircle } from 'lucide-react'
...
/** Compact bar chart that expands below the clicked stat */
...
function ProgressTasks({ entityId, onRetry }: { entityId: string; onRetry?: () => void }) {
  ...
  return (
    <div className="topic-detail rise">
      ...
      <div ref={statsRef} className="topic-detail-stats pop" style={{ animationDelay: '80ms' }}>
        ...
        <div className="topic-detail-stat-sep" />
        {(() => {
          const interval = Number((topic.config as Record<string, unknown>)?.schedule_interval_hours) || 6
          const nextAt = topic.last_crawl_at ? new Date(new Date(topic.last_crawl_at).getTime() + interval * 3600_000) : null
          ...
          return (
            <div className="topic-detail-stat" title={`Crawls every ${interval}h${topic.status === 'paused' ? ' (paused)' : ''}`}>
              <span className="topic-detail-stat-label">Next</span>
              <span className={`topic-detail-stat-value${due ? ' due' : ''}`}>
                {topic.status === 'paused' ? 'Paused' : dueText}
              </span>
        ...
        >
          <BarChart3 size={13} />
          <span className="topic-detail-stat-num">{fmtNum(postsTotal ?? topic.total_contents)}</span>

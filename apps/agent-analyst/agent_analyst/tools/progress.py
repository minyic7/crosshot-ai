"""set_progress_stage — update topic/user progress state in Redis."""
...
DONE_TTL = 30  # brief window for "done" badge visibility
...
async def set_progress_stage(
    ...
    """Update the progress stage for a topic or user."""
    ...
    await redis_client.hset(progress_key, mapping=mapping)
    # Short TTL for "done" so badge auto-disappears; longer for active phases
    ttl = DONE_TTL if phase == "done" else PROGRESS_TTL
    await redis_client.expire(progress_key, ttl)
    ...
    if phase == "crawling" and total is not None:
        ...
        await redis_client.set(pending_key, total, ex=PROGRESS_TTL)
        await redis_client.set(on_complete_key, json.dumps(on_complete_payload), ex=PROGRESS_TTL)
